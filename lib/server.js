...
var SPDYProxy = function(options) {
  ...
  function handleRequest(req, resOrSocket) {
    var socket = (req.method == 'CONNECT') ? resOrSocket : resOrSocket.socket;
    console.log("%s:%s".yellow + " - %s - " + "stream ID: " + "%s".yellow,
      ...
      req.method, resOrSocket.streamID || socket.streamID
    );

    var dispatcher = function(req, resOrSocket) {
      req.method == 'CONNECT' ? handleSecure(req, resOrSocket) : handlePlain(req, resOrSocket);
    }
    ...
    if(typeof self._authHandler == 'object') { // an AuthHandler is defined
      ...
      self._authHandler.authUser(username, password, function(authPassed) {
        if (authPassed)
          return dispatcher(req, resOrSocket);
